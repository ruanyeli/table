 ...
 * @Date:   2017-07-03 09:55:17
 * @Last modified by:   yzf
 * @Last modified time: 2017-07-03 09:55:21
 *
 * a fork of rc-table
 */
...
export default class Table extends React.Component {
  ...
  getHeaderRows(columns, currentRow = 0, rows) {
    ...
    columns.forEach(column => {
      ...
      const headerCellEvent = column.onHeaderCell ? column.onHeaderCell(column) : {};
      const cell = {
        key: column.key,
  ...
  getExpandedRow(key, content, visible, className, fixed) {
    const { prefixCls, expandIconAsCell, onRowMouseEnter, onRowMouseLeave, scroll, size, lazyload, lazyloadHeight } = this.props;
    let colCount;
    if (fixed === 'left') {
  ...
  getTable(options = {}) {
    ...
    if (scroll.y) {
      ...
      bodyStyle.overflow = bodyStyle.overflow || 'scroll';
      useFixedHeader = true;
    ...
    if (useFixedHeader && showHeader) {
      headTable = (
        <div
          ...
          ref={fixed ? null : headTable => this.headTable = headTable}
          style={headStyle}
    ...
    if (fixed && columns.length) {
      ...
      bodyTable = (
        <div
          ...
          style={{ ...bodyStyle, overflow: 'inherit' }}
        >
          <div
            className={`${prefixCls}-body-inner`}
            style={innerBodyStyle}
            ref={_ref => this[refName] = _ref}
            onMouseOver={this.detectScrollTarget}
            onTouchStart={this.detectScrollTarget}
            onScroll={this.handleBodyScroll}
          >
  ...
  saveTableNodeRef = node => this.tableNode = node;
  ...
  render() {
    ...
    return (
      <div ref={this.saveTableNodeRef} className={className} style={props.style}>
        {this.getTitle()}
