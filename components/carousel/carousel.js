...
class Carousel extends React.Component {
  ...
  componentDidMount () {
    ...
    this.dotsClick(activeKey, 0);
    this.mounted = true
  }

  componentWillReceiveProps(nextProps) {
    if ('activeKey' in nextProps) {
      if (nextProps.activeKey !== this.state.current) {
        // this.setState({
        //   current: nextProps.activeKey
        // });
        this.dotsClick(nextProps.activeKey);
      }
    }
  }
  ...
  prepareAutoSlide () {
    ...
    if (this.props.loop && this.props.auto && this.mounted && !('activeKey' in this.props)) {
      // 自动滑动只在循环模式下且未设置acitiveKey下进行
  ...
  prepareSiblingFrames () {
    const siblings = {
      current: this['f' + this.getFrameId()],
      prev: this['f' + this.getFrameId('prev')],
      next: this['f' + this.getFrameId('next')]
    }
  ...
  transitFramesTowards (direction) {
    ...
    if (this.state.current !== newCurrentId) {
      this.props.onChange(this.state.current, newCurrentId)
      // console.log('onChange', this.state.current, newCurrentId);
    }
  ...
  /**
   * @param {*} index
   * @param {*} delay 可选 传0 则为立即执行 frame 切换，不要动画
   * @memberof Carousel
   */
  dotsClick (index, delay) {
    const currentId = this.getFrameId();
    const { axis } = this.props
    const { frames } = this.state
    index = index % frames.length
    const duration = isNaN(delay) ? this.props.duration : delay;
    if (currentId === index) return
    const movingInFrame =this['f' + index]
    const current = this['f' + currentId]
    if (axis === 'x') {
    ...
    this.props.onChange(currentId, index);
  }

  saveWrapperRef = wrapper => this.wrapper = wrapper;

  render () {
    const { frames, current } = this.state
    const { prevAndNext, axis, loop, auto, interval } = this.props
    // console.log('frames', frames, styles);  
    return (
      <div
      ...
      >
        {
          frames.map((frame, i) => {
            ...
            return <div ref={r => this['f' + i] = r} key={i} style={frameStyle}>{frame}</div>
          })
        }
        <Dots
          index={current}
