...
class TreeNode extends Component {
  ...
  renderNoopSwitcher(props) {
    ...
    if (props.isLeaf && props.showLine) {
      switcherCls[`${prefixCls}-switcher-line-child`] = true;
    }
    return <span className={cn(switcherCls)}></span>;
  }

  saveCheckboxRef = checkbox => this.checkbox = checkbox;
  saveSelectHandleRef = selectHandle => this.selectHandle = selectHandle;
  ...
  renderCheckbox(props) {
    ...
    if (props.disabled || props.disableCheckbox) {
      ...
      return <span ref={this.saveCheckboxRef} className={cn(checkboxCls)}></span>
    }
    return <span ref={this.saveCheckboxRef} className={cn(checkboxCls)} onClick={this.onCheck}></span>
  }
  ...
  render() {
    ...
    const selectHandle = () => {
      const icon = (props.showIcon || (props.loadData && dataLoading)) ? <span className={cn(iconEleCls)}>{
        dataLoading ? <span className='loading'></span> : null
      }</span> : '';
      const content = <span className={`${prefixCls}-title`}>{title}</span>;
      return (
        <span {...domProps} ref={this.saveSelectHandleRef} title={typeof title === 'string' ? title : ''}>
          {icon}
          {content}
        </span>
      )
    ...
    return (
      <li {...liProps} className={cn(props.className, disabledCls, dragOverCls, filterCls)}>
        {
          canRenderSwitcher ? this.renderSwitcher(props, expandedState) : this.renderNoopSwitcher(props)
        }
        {
