...
export default class MonthTable extends React.Component {
  ...
  renderBody(momentLocale) {
    // moment.locale(momentLocale);
    const { value, onSelect, prefixCls, disabledDate } = this.props;
    const months = chunk(momentLocale === 'zh-CN' ? moment.months() : moment.monthsShort(), 4);
  ...
  render() {
    ...
    return <table className={prefixCls} cellSpacing="0" cellPadding="0">
      <LocaleReceiver componentName="DatePicker">
        {
          local => this.renderBody(local.momentLocale)
        }
      </LocaleReceiver>
    </table>
  }
