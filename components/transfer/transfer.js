...
export default class Transfer extends Component {
  ...
  static propTypes = {
    ...
    sourceDisabledKeys: PropTypes.array,
    targetDisabledKeys: PropTypes.array,
  ...
  constructor(props) {
    ...
    if ('render' in props) {
      warning(
        ...
        'props render should be a function.'
      );
    }
    this.state = {
      value,
  ...
  toggleState = (key, _key, type, isSelected) => {
    ...
    const eventKey = `on${_upperFirst(type)}SelectedKeysChange`;
    ...
    for (let i = 0; i < _keyArr.length; i++) {
      ...
      checkList[thisNode.key] = _cloneDeep(thisNode.children);
    }
    ...
    if (isSelected) {
      ...
      _remove(nextSelectedKeys, item => doList.indexOf(item) >= 0);
    } else {
      // 去除选择
      nextSelectedKeys = doList.concat(selectedKeys);
    }
    nextSelectedKeys = _uniq(nextSelectedKeys);
    const disabledKeys = this.props[`${type}DisabledKeys`];
    _remove(nextSelectedKeys, key => disabledKeys.includes(key));
  ...
  selectedAll = (type, disabled, isAllSelected, dataList) => {
    ...
    if (type === 'target') {
      if (isAllSelected) {
        ...
        this.setState({
          targetSelectedKeys: this.filterDisabled(getAllKey(dataList), type)
        });
      }
    } else {
      if (isAllSelected) {
        ...
        this.setState({
          sourceSelectedKeys: this.filterDisabled(getAllKey(dataList), type)
        });
      }
    }
  }

  filterDisabled = (list, type) => {
    const disabledKeys = this.props[`${type}DisabledKeys`];
    _remove(list, i => disabledKeys.includes(i));
    return list
  }
  ...
  changeValue = (type) => {
    ...
    if (type === 'target') {
      ...
      newValue = _remove(value, item => targetSelectedKeys.indexOf(item) < 0);
      this.setState({
        targetSelectedKeys: []
      });
    }
    newValue = _uniq(newValue);
  ...
  renderTargetItems = (locale) => {
    const { value, dataSource } = this.state;
    const {
      target
    } = getListData(dataSource, value);
    return this.renderItem(locale, target, 'target')
  }

  renderSourceItems = (locale) => {
    const { value, dataSource } = this.state;
    const {
      source
    } = getListData(dataSource, value);
    return this.renderItem(locale, source, 'source')
  }

  renderItem = (locale, datas, type, level = 0) => {
    const { prefixCls, notFoundContent, rowKey, render, sourceDisabledKeys, targetDisabledKeys } = this.props;
    const customerRender = typeof render === 'function';
    if (datas.length > 0) {
      return datas.map((item, index) => {
        const isDisabled = type == 'source' && sourceDisabledKeys.includes(item.key) || type == 'target' && targetDisabledKeys.includes(item.key);
        ...
        const itemClass = classnames(`${prefixCls}-checkbox`, {
          ...
          [`${prefixCls}-checkbox-disabled`]: isDisabled
        });
        return (
          <div key={item[rowKey]}>
            <div
              className={classnames(`${prefixCls}-con-item`, { [`${prefixCls}-con-item-disabled`]: isDisabled })}
              onClick={() => {
                !isDisabled&&this.toggleState(item.key, item._key, type, typeArr.indexOf(item.key) >= 0)
              }}
              style={{paddingLeft: level * 23 + 15}}
            >
              <span className={itemClass}></span>{customerRender ? render(item) : item.title}
            </div>
            { item.children && this.renderItem(locale, item.children, type, level + 1) }
          </div>
        );
      })
    } else if (level === 0) {
      return <div className="null">{notFoundContent || locale.notFoundContent}</div>
    }
  }

  render() {
    const { prefixCls, operations, titles, className, height } = this.props;
    ...
    return (<div className={`${prefixCls} ${className || ''}`} style={this.props.style}>
      <div className={`${prefixCls}-box ${prefixCls}-source`}>
        ...
        <div className={`${prefixCls}-con`} style={{height: height - 36}}>
      ...
      <div className={`${prefixCls}-middle`}>
        <div className={`${prefixCls}-middle-inner`}>
          <div>
            ...
            >{ operations[0] == '>' ? <Icon type='enter'/> : operations[0] }</Button>
          </div>
          <div>
            <Button
              type='primary'
            ...
            >{ operations[1] == '<' ? <Icon type='return'/> : operations[1] }</Button>
          </div>
        </div>
      </div>
