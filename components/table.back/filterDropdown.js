...
export default class FilterMenu extends React.Component {
  ...
  renderFilterConfirm = (loca) => {
    const { locale } = this.props;
    return locale.filterConfirm || loca.filterConfirm
  }

  renderFilterReset = (loca) => {
    const { locale } = this.props;
    return locale.filterReset || loca.filterReset
  }

  renderFilterIcon = (loca) => {
    ...
    return filterIcon ? React.cloneElement(filterIcon, {
      title: locale.filterTitle || loca.filterTitle,
      className: classNames(filterIcon.className, {
        [`${prefixCls}-icon`]: true
      })
    }) : <Icon title={locale.filterTitle || loca.filterTitle} type="screening" className={dropdownSelectedClass} />;
  }
    ...
    const menus = column.filterDropdown ? (
      ...
      <FilterDropdownMenuWrapper className={`${prefixCls}-dropdown`}>
        ...
        <div className={`${prefixCls}-dropdown-btns`}>
          ...
          >
            {this.renderFilterConfirm(loca)}
          </a>
          <a
            className={`${prefixCls}-dropdown-link clear`}
            onClick={this.handleClearFilters}
          >
            {this.renderFilterReset(loca)}
          </a>
        </div>
      </FilterDropdownMenuWrapper>
    );
