...
export default class SelectionCheckboxAll extends React.Component {
  ...
  renderMain = (loca) => {
    const { disabled, prefixCls, selections, getPopupContainer, locale } = this.props;
    ...
    let defaultSelections = [{
      key: 'all',
      text: locale.selectAll || loca.selectAll,
      onSelect: () => {}
    }, {
      key: 'invert',
      text: locale.selectInvert || loca.selectInvert,
      onSelect: () => {}
    }];;

    if (selections) {
      let newSelections = Array.isArray(selections) ? defaultSelections.concat(selections) : defaultSelections;
