...
export default class Calendar extends React.Component {
  ...
  onPrev = () => {
    ...
    if (!disabledDate || !disabledDate(newValue)) {
      isChange = true;
    } else if (selectingType === DAY || selectingType === MONTH) {
      selectingType === MONTH ? newValue.endOf('years') : newValue.endOf('month');
      if (!disabledDate || !disabledDate(newValue)) {
        isChange = true;
      }
    }
  ...
  onNext = () => {
    ...
    if (!disabledDate || !disabledDate(newValue)) {
      ...
      selectingType === MONTH ? newValue.startOf('years') : newValue.startOf('month');
      if (!disabledDate || !disabledDate(newValue)) {
        isChange = true;
      }
    }
  ...
  renderHeader = () => {
    ...
    return (
      <LocaleReceiver componentName="DatePicker">
        {
          local => {
            // moment.locale(local.momentLocale);
            return <div className={`${prefixCls}-header`}>
              ...
              <div className={`${prefixCls}-header-btn`} onClick={this.onSelectingTypeChange}>
                {
                  selectingType === TIME ? value.format(local.dateFormat) : (
                    selectingType === DAY ? value.format(local.monthFormat) : (
                      selectingType === MONTH ? value.format(local.yearFormat) : `${Math.floor(value.year() / 10) * 10}-${Math.ceil(value.year() / 10) * 10 - 1}`
                    )
                  )
                }
              </div>
  ...
  renderFooter = () => {
    ...
    return <LocaleReceiver componentName="DatePicker">
      {
        local => <div className={`${prefixCls}-footer`}>
          <div className={`${prefixCls}-footer-default`} onClick={
          ...
          {
            selectingType === TIME ? local.now : (
              (selectingType === DAY || calendarType >= DAY_PICKER) ? local.today : (
                (selectingType === MONTH || calendarType >= MONTH_PICKER) ? local.currentMonth : local.currentYear
              )
            )
          }
          </div>
          {
            !!renderExtraFooter && <div className={`${prefixCls}-footer-extra`}>
              {
